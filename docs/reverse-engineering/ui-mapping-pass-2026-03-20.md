...
- The retail `setplayermodel` and `setplayerhead` handlers write `"model"` and `"headmodel"` through the UI display context. The reconstructed feeder path now mirrors the adjacent retail model-list behavior as well: `ui_shared.c` passes the listbox item cvar into `feederSelection`, `FEEDER_HEADS` / `FEEDER_Q3HEADS` resolve rows through the validated player-model catalog, and selection writes either the supplied item cvar (for force-model listboxes) or the paired `"model"` / `"headmodel"` cvars.
