...
| ZMQ stats publisher and remote RCON service | High parity | `sv_zmq.c`, `sv_game.c`, `sv_main.c`, `sv_init.c`, `common.c` | mapping rounds `94`..`97`; HLIL strings `zmq_rcon_enable`, `zmq_stats_enable`, `zmq RCON socket`, `zmq PUB socket`, `game.end`, `MATCH_REPORT` | Writable source now owns the retained `idZMQ` cvar/runtime split, per-frame password refresh, RCON pump/output broadcast, stats publisher lifecycle, typed publication helpers, retail cvar flag contract, and compact live PUB frame format, with repo-policy fallbacks when live transport is unavailable. |
...
1. Writable source now defines the mapped `zmq_*` cvar surface in `sv_zmq.c`, including `zmq_rcon_enable`, `zmq_stats_enable`, the password slots, and the address/port companions. The bind-control cvars use the HLIL-observed `CVAR_INIT` contract, while the password cvars remain archived and frame-refreshable.
...
4. The 2026-06-05 cvar-flag and publication-frame recheck against HLIL `0x004F3F80`, `0x004F4210`, `0x004F4B20`, and `0x004F4C30` closed the remaining ZMQ stats/RCON mismatches: `zmq_rcon_enable`, `zmq_stats_enable`, `zmq_rcon_ip`, `zmq_rcon_port`, `zmq_stats_ip`, and `zmq_stats_port` are init-only; `zmq_stats_password` and `zmq_rcon_password` are archived without the extra non-retail protected bit; live PUB messages send the compact JSON object without the local transcript newline.

Conclusion:

- `SV-G02` is closed. The retained `idZMQ` host/runtime now exists in writable source with the mapped cvar, lifecycle, publication, and RCON ownership seams. Live socket use remains policy-gated by `QL_BUILD_ONLINE_SERVICES`; that fallback boundary is intentional and separate from the retail source-surface reconstruction.
