...
- Disconnected WebUI requests now claim fullscreen ownership only after the browser owns input or has a drawable surface. Pending Awesomium startup still advertises the requested live-browser load, but the native disconnected menu is allowed to re-arm `UIMENU_MAIN`, refresh, draw its cursor, and receive clicks until the live surface is actually usable.
- The live Awesomium adapter now resolves `awesomium_process.exe` and `web.pak` from `fs_homepath` first, falling back to `fs_basepath`. `WebConfig::package_path` receives the selected root, while `DataPakSource` receives the retail package member name `web.pak`.
...
- A live Awesomium startup that never produces a contentful `BitmapSurface` is now bounded. After 180 missing-surface frames, the host logs the last Awesomium state, shuts down the live WebCore path, clears browser keycatch/cvars, latches `loadFailed`, and refreshes the bridge so the native menu can regain control instead of freezing behind a permanently pending WebUI owner. Successful native map and factory catalog transfers are also one-shot per document until invalidated, preventing repeated sync churn while the menu is idle.
- Repeated `web_showBrowser` / `web_changeHash` commands for the already loaded target are idempotent while the retained WebView is alive. The command owner keeps the view visible/focused without reopening the same URL, so native menu scripts cannot restart the load and catalog-sync cycle every frame while Awesomium is still settling.
...
- `CL_Awesomium_CreateSession` constructs the data source with `DataPakSource("web.pak")` after verifying the file exists in the selected root. The resolved path remains validation and diagnostics only, while `WebConfig::package_path` points Awesomium at the root that contains the package.
