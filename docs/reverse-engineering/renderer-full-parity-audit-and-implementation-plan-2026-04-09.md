...
| Memory-image ingestion and live resource registration | Closed | `tr_image.c`, `tr_backend.c`, `tr_local.h`, `cl_main.c`, `cl_steam_resources.c` | mapping rounds 42, 312, 313, 314, and 315; aliases `GL_BindToTarget`, `R_CreateImageWithTarget`, `R_DetectImageTypeFromMemory`, `R_LoadImageFromMemory`, `generateHashValue`; parity gate `RG-G02` | The retail in-memory image lane remains reconstructed and actively used by launcher or Steam resource ingestion; the image constructor now keeps the retail browser RGBA override, shared target-aware bind helper, texture-id fallback, memory-loader clamp policy, image-list diagnostics, hash-helper ownership, and shader color-correct upload-gamma ownership. |
