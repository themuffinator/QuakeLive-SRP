...
| Memory-image ingestion and live resource registration | Closed | `tr_image.c`, `tr_backend.c`, `tr_local.h`, `cl_main.c`, `cl_steam_resources.c` | mapping rounds 42, 312, 313, 314, 315, 316, and 317; aliases `GL_BindToTarget`, `RBPP_ColorCorrectEnabled`, `R_CreateImageWithTarget`, `R_DetectImageTypeFromMemory`, `R_LoadImageFromMemory`, `generateHashValue`; parity gate `RG-G02` | The retail in-memory image lane remains reconstructed and actively used by launcher or Steam resource ingestion; the image constructor now keeps the retail browser RGBA override, shared target-aware bind helper, texture-id fallback, memory-loader clamp policy, image-list diagnostics, hash-helper ownership, and direct shader color-correct upload-gamma ownership. |
| Post-process and color correction | Closed | `tr_backend.c`, `tr_cmds.c`, `tr_init.c`, `tr_local.h`, `tr_public.h`, `tr_shader.c` | HLIL strings for `brightpass`, `downsample1`, `blurvertical`, `blurhoriz`, `combine`, and `colorcorrect`; command IDs `9`, `10`, and `11`; private tail slots `data_5878c0`..`data_5878cc`; mapping rounds 316, 317, 318, 319, 320, 321, 322, 323, 324, 325, 326, 327, 328, and 341; parity gate `RG-G03` | The recovered shader-backed rectangle-texture pipeline still matches the committed retail shader family, including the promoted `RBPP_ColorCorrectEnabled` predicate used by gamma/image upload paths, the promoted `RBPP_BloomEnabled` predicate that keeps the scene render target owned by bloom, the reconstructed backend command ABI with command-payload texture/program handle consumption, the private refexport tail wiring for bloom capture, bloom command submission, restart, and five-float bloom uniform setting, the corrected retail combine-uniform slot order for bloom saturation, scene saturation, bloom intensity, and scene intensity, the separated color-correct cvar-to-uniform helper matching `sub_43CD60`, the frame-begin live modified-latch refresh path that calls the color-correct and bloom uniform helpers before the later gamma color-mapping branch, the split between init-time color-correct uniform seeding and live browser-active gamma/contrast suppression, the retired legacy `GL_TEXTURE_2D` framebuffer/scratch-bloom duplicate in favor of the retail `RBPP_CreateRenderTarget` rectangle-texture owner, the retail grouped bloom teardown order for programs, textures, framebuffers, and renderbuffers, the retail eight-entry depth-stencil renderbuffer cache used by post-process target creation, the return-valued `GL_CheckErrors` / post-effect link helper wiring used by texture, framebuffer, and program setup, and the split framebuffer-only versus shader/uniform procedure gates. |
...
| Win32 renderer host glue | Closed | `win_wndproc.c`, `win_glimp.c`, `win_syscon.c`, `win_main.c`, `win_local.h` | mapping rounds 98, 100, and 342; runtime capture artifact; parity gates `RG-G04` and `RG-G07` | Resize sync, maximized-window retention, loading-window ownership, and Win32 OpenGL startup ICD fallback modes `5` then `12` remain aligned with the committed retail host. |
...
- `RG-G03` stays closed. The shader-backed post-process family, recovered `r_contrast` lane, command-buffer ABI with queued texture/program payload consumption, private refexport tail for color-correct, bloom, scene-target binding, bloom uniform setting, the single rectangle-texture framebuffer owner, grouped bloom teardown ordering, depth-stencil renderbuffer cache, GL-error/link helper wiring, and split framebuffer/shader procedure gates remain in writable source.
- `RG-G04` stays closed. The retail Win32 resize, restart, loading-window behavior, and OpenGL startup ICD fallback sequence remain reconstructed and runtime- or corpus-backed.
