...
| Mouse input | `CL_MouseEvent` – `src/code/client/cl_input.c` | `VM_Call(uivm, UI_MOUSE_EVENT, dx, dy);` | `void UI_MouseEvent(int dx, int dy);` | Preserve the recovered raw event payload after the advertisement-delay, `cg_ignoreMouseInput`, and browser-keycatcher gates. 【F:src/code/client/cl_input.c†L368-L394】 |
...
| Mouse input | `CL_MouseEvent` – `src/code/client/cl_input.c` | `VM_Call(cgvm, CG_MOUSE_EVENT, dx, dy);` | `void CG_MouseEvent(int dx, int dy);` | Preserve the recovered raw event payload after the advertisement-delay, `cg_ignoreMouseInput`, and browser-keycatcher gates. 【F:src/code/client/cl_input.c†L368-L394】 |
