...
* **Environment separation:** Keep 32-bit build outputs isolated (for example, `build\win32-native\`) and confirm MSBuild is invoked with `/p:Platform=Win32` so the correct compiler/linker frontends are used.
...
4. **MSBuild targeting:** From a Visual Studio x86 Native Tools prompt, execute `msbuild src\code\quake3.sln /t:qagamex86 /p:Platform=Win32 /p:Configuration=Debug` and verify outputs land in `build\win32-native\qagamex86\Debug`. Any `“64-bit tools are not available”` warnings indicate the wrong developer prompt was used.
