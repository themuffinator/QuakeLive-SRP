...
| `g_knockback`, `g_knockback_*`, `g_knockback_*_self`, `g_knockback_z(_self)`, `g_max_knockback`, `g_knockback_cripple` | Registers global and per-weapon knockback scalars, self-damage tweaks, the positive knockback clamp, table-retained z rows, and the movement-blocking timer floor. | Expand Quake III's single `g_knockback` cvar by introducing a struct of weapon-specific scalars and self-hit overrides; consume the observed scalars in `G_Damage`, keep `g_knockback_z(_self)` as table-parity rows until a retail consumer is documented, and feed `g_knockback_cripple` only into the `PMF_TIME_KNOCKBACK` timer floor. |
...
- **Movement and combat integration**: Adjust knockback multipliers while computing weapon kick in `G_Damage`, preserving signed knockback, `g_knockback_*_self` overrides, the `g_max_knockback` positive clamp, and the `PMF_TIME_KNOCKBACK` blocking timer consumed by shared pmove.
