...
### HUD gating behavior
- The HUD loader now inspects `cg_hudFiles` scripts for `comp_*.menu` references and records whether the Quake Live competitive HUD assets are available for menu/feeder fallbacks, while the native chat geometry follows the retail match-summary split instead of a source-only override cvar.【F:src/code/cgame/cg_main.c†L1829-L1876】【F:src/code/cgame/cg_main.c†L169-L212】
