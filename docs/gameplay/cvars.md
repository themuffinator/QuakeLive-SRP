...
| `g_freezeThawTime` | `2000` | `CVAR_GAMERULE` | Cached into `level.freezeConfig.thawTime`, used as the total ally-assist thaw budget, and split into the retail low-bit `EF_DEAD`/`EF_TICKING` thaw-progress buckets while frozen.【F:src/code/game/g_active.c†L2390-L2403】【F:src/code/game/g_client.c†L145-L263】 |
| `g_freezeThawTick` | `1` | `CVAR_GAMERULE` | Cached into `level.freezeConfig.thawTick` and used as the non-zero gate for thaw-progress tick events; retail thaw progress itself follows the frame delta against the remaining `g_freezeThawTime` budget.【F:src/code/game/g_active.c†L2390-L2403】【F:src/code/game/g_client.c†L145-L263】 |
| `g_freezeThawRadius` | `96` | `CVAR_GAMERULE` | Sets the helper-search radius used to build the retail-style `trap_EntitiesInBox` thaw query, including the line-of-sight gate when through-surface thawing is disabled.【F:src/code/game/g_active.c†L2390-L2403】【F:src/code/game/g_freeze.c†L244-L303】 |
