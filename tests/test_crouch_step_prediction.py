...
def _load_library(lib_path: Path) -> ctypes.CDLL:
    ...
    library.QLR_ApplyStepJump.argtypes = [
        ctypes.c_int,
        ctypes.c_int,
        ctypes.c_int,
        ctypes.c_int,
...
def test_step_jump_requires_jump_input(tmp_path: Path) -> None:
    ...
    assert no_input_velocity == pytest.approx(initial_velocity.value, rel=1e-6)

    boosted_velocity = library.QLR_ApplyStepJump(
        ctypes.c_int(0),
        ctypes.c_int(20),
        ctypes.c_int(1),
        ctypes.c_int(1),
    ...
    negative_step_velocity = library.QLR_ApplyStepJump(
        ctypes.c_int(0),
        ctypes.c_int(20),
        ctypes.c_int(1),
        ctypes.c_int(1),
...
@pytest.mark.skipif(os.name == "nt", reason="MSVC build configuration not supported in tests")
def test_crouch_step_jump_obeys_ducked_gate(tmp_path: Path) -> None:
    lib_path = _build_test_library(tmp_path)
    library = _load_library(lib_path)
    ...
    jump_velocity = ctypes.c_float(275.0)
    jump_velocity_max = ctypes.c_float(700.0)
    step_jump_velocity = ctypes.c_float(48.0)

    no_duck_velocity = library.QLR_ApplyStepJump(
        ctypes.c_int(0),
        ctypes.c_int(20),
        ctypes.c_int(1),
        ctypes.c_int(1),
    ...
    assert no_duck_velocity == pytest.approx(initial_velocity.value, rel=1e-6)

    crouched_velocity = library.QLR_ApplyStepJump(
        ctypes.c_int(1),
        ctypes.c_int(20),
        ctypes.c_int(1),
        ctypes.c_int(1),
    ...
    assert disabled_crouch_step_velocity == pytest.approx(initial_velocity.value, rel=1e-6)
