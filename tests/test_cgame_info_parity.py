...
def test_cg_draw_information_uses_retail_title_and_author_rules() -> None:
	...
	assert "static void CG_DrawLoadingTextAtTop( float x, float y, int fontIndex, float scale, const char *text, int style )" in cg_info
	assert "height = CG_Text_HeightExt( text, scale, 0, fontIndex );" in cg_info
	assert "CG_DrawLoadingText( x, y + height, fontIndex, scale, text, style );" in cg_info
	...
	assert "CG_DrawLoadingTextAtTop( LOADING_TITLE_X, LOADING_TITLE_Y, FONT_DEFAULT, LOADING_TITLE_SCALE, title," in cg_info
	...
	assert 'CG_DrawLoadingTextAtTop( LOADING_TITLE_X, LOADING_AUTHOR_Y, FONT_SANS, LOADING_META_SCALE, author,' in cg_info
	assert 'CG_DrawLoadingTextAtTop( LOADING_GAMETYPE_RIGHT_X - width, LOADING_GAMETYPE_Y, FONT_DEFAULT, LOADING_GAMETYPE_SCALE,' in cg_info
