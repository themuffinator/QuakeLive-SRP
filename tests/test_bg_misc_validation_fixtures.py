...
def bg_misc_validation_harness(tmp_path_factory: pytest.TempPathFactory) -> ctypes.CDLL:
	...
	library.QLR_TouchJumpPadOverwritesVelocity.argtypes = []
	library.QLR_TouchJumpPadOverwritesVelocity.restype = ctypes.c_int
...
def test_predictable_event_and_jump_pad_fixtures_cover_shared_event_transport(
	...
	assert bg_misc_validation_harness.QLR_TouchJumpPadOverwritesVelocity() == 800
	assert _packed_event_snapshot(
...
def test_jump_pad_cache_clear_policy_is_shared_by_server_and_prediction() -> None:
	...
	assert "BG_ShouldClearJumpPadLaunch" not in bg_public
	assert "BG_ShouldClearJumpPadLaunch" not in bg_misc
	assert "if ( ent->client->ps.jumppad_frame != ent->client->ps.pmove_framecount ) {" in g_active
	assert "if ( cg.predictedPlayerState.jumppad_frame != cg.predictedPlayerState.pmove_framecount ) {" in cg_predict
	assert g_active.index("Pmove (&pm);") < g_active.index("G_TouchTriggers( ent );")
	assert cg_predict.index("Pmove (&cg_pmove);") < cg_predict.index("CG_TouchTriggerPrediction();")
	assert "BG_TouchJumpPad( &cg.predictedPlayerState, ent );" in cg_predict
