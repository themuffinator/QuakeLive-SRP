...
void R_PerformanceCounters( void ) {
	...
	if ( r_speeds->integer == 1 ) {
		...
		if ( backEnd.pc.c_smallBatchSkips || backEnd.pc.c_largeBatchSkips ) {
			ri.Printf( PRINT_ALL, "skipped batches small:%i large:%i (%i idx)\n",
				backEnd.pc.c_smallBatchSkips, backEnd.pc.c_largeBatchSkips, backEnd.pc.c_skippedIndexes );
		}
