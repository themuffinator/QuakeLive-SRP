...
static qboolean ParseShader( char **text )
{
	...
	while ( 1 )
	{
      ...
      if (token[0]) {
		...
		// disable vertex-light optimization collapse
		else if ( !Q_stricmp( token, "novlcollapse" ) )
		{
			shader.noVertexLightCollapse = qtrue;
			continue;
		}
		// polygonOffset
		...
		while ( 1 ) {
			...
			case RC_DRAW_SURFS:
				...
				for( i = 0, drawSurf = ds_cmd->drawSurfs; i < ds_cmd->numDrawSurfs; i++, drawSurf++ ) {
					R_DecomposeSort( drawSurf->sort, &entityNum, &drawShader, &fogNum, &dlightMap );
...
static shader_t *FinishShader( void ) {
	...
	//
	if ( stage > 1 && !shader.noVertexLightCollapse &&
...
void	R_ShaderList_f (void) {
	...
	for ( i = 0 ; i < tr.numShaders ; i++ ) {
		if ( ri.Cmd_Argc() > 1 ) {
			listedShader = tr.sortedShaders[i];
		} else {
			listedShader = tr.shaders[i];
		}

		ri.Printf( PRINT_ALL, "%i ", listedShader->numUnfoggedPasses );

		if (listedShader->lightmapIndex >= 0 ) {
			ri.Printf (PRINT_ALL, "L ");
		} else {
			ri.Printf (PRINT_ALL, "  ");
		}
		if ( listedShader->multitextureEnv == GL_ADD ) {
			ri.Printf( PRINT_ALL, "MT(a) " );
		} else if ( listedShader->multitextureEnv == GL_MODULATE ) {
			ri.Printf( PRINT_ALL, "MT(m) " );
		} else if ( listedShader->multitextureEnv == GL_DECAL ) {
			ri.Printf( PRINT_ALL, "MT(d) " );
		} else {
			ri.Printf( PRINT_ALL, "      " );
		}
		if ( listedShader->explicitlyDefined ) {
			ri.Printf( PRINT_ALL, "E " );
		} else {
			ri.Printf( PRINT_ALL, "  " );
		}

		if ( listedShader->optimalStageIteratorFunc == RB_StageIteratorGeneric ) {
			ri.Printf( PRINT_ALL, "gen " );
		} else if ( listedShader->optimalStageIteratorFunc == RB_StageIteratorSky ) {
			ri.Printf( PRINT_ALL, "sky " );
		} else if ( listedShader->optimalStageIteratorFunc == RB_StageIteratorLightmappedMultitexture ) {
			ri.Printf( PRINT_ALL, "lmmt" );
		} else if ( listedShader->optimalStageIteratorFunc == RB_StageIteratorVertexLitTexture ) {
			ri.Printf( PRINT_ALL, "vlt " );
		} else {
			ri.Printf( PRINT_ALL, "    " );
		}

		if ( listedShader->defaultShader ) {
