...
=================
*/
static qboolean Com_CredentialValueContainsUnsafeWhitespace( const char *value ) {
...
static void Com_WriteCredentialFile( fileHandle_t f, const ql_auth_credential_t *credential ) {
        ...
        if ( !QL_FormatCredentialForStorage( credential, label, sizeof( label ), storedValue, sizeof( storedValue ) ) ) {
                return;
        }
...
static void Com_WriteCDKey( const char *filename, const char *ikey ) {
        ...
        if ( Com_CredentialValueContainsUnsafeWhitespace( credential.value ) ) {
                Com_Printf( "Ignoring credential with embedded newlines for %s.\n", filename );
                return;
        }
        ...
        Com_WriteCredentialFile( f, &credential );
