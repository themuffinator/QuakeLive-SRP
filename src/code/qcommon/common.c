...
#define QL_CONFIG_HARDWARE_HEADER "// Hardware cfg - Generated by QUAKE LIVE. Do not modify\n"
#define QL_CONFIG_REPLICATE_HEADER "// Replicate cfg - Generated by QUAKE LIVE. Do not modify\n"
#define QL_CDKEY_PLACEHOLDER "                "
#define QL_CDKEY_FILE_NOTE_0 "\n// generated by quake, do not modify\r\n"
#define QL_CDKEY_FILE_NOTE_1 "// Do not give this file to ANYONE.\r\n"
#define QL_CDKEY_FILE_NOTE_2 "// id Software and Activision will NOT ask you to send this file to them.\r\n"
...
char	cl_cdkey[QL_AUTH_MAX_CREDENTIAL_STORAGE] = QL_CDKEY_PLACEHOLDER;
char	cl_cdkey_mod[QL_AUTH_MAX_CREDENTIAL_STORAGE] = QL_CDKEY_PLACEHOLDER;
...
static void Com_WriteCDKey( const char *filename, const char *ikey ) {
	...
	FS_Write( key, CDKEY_LEN, f );
	FS_Printf( f, QL_CDKEY_FILE_NOTE_0 );
	FS_Printf( f, QL_CDKEY_FILE_NOTE_1 );
	FS_Printf( f, QL_CDKEY_FILE_NOTE_2 );
...
//	Swap_Init ();
	...
	// Memory allocators are sized before the usual post-config startup-variable pass,
	// so bootstrap probe-critical allocator cvars from the command line first.
	...
	if ( !Com_SafeMode() ) {
		Cbuf_AddText ("exec " QL_CONFIG_HARDWARE_FILE "\n");
	}

#ifndef DEDICATED
	Cbuf_AddText ("exec " QL_CONFIG_REPLICATE_FILE "\n");
#else
...
void Com_WriteConfiguration( void ) {
	...
	if ( !com_dedicated->integer ) {
		Com_WriteConfigToFile( QL_CONFIG_HARDWARE_FILE, QL_CONFIG_REPLICATE_FILE, qfalse );
	}
...
void Com_WriteConfig_f( void ) {
	...
	if ( Cmd_Argc() != 2 && Cmd_Argc() != 3 ) {
		Com_Printf( "Usage: writeconfig <filename> [<rep filename>]\n" );
		return;
	}

	Q_strncpyz( hardwareFilename, Cmd_Argv(1), sizeof( hardwareFilename ) );
	COM_DefaultExtension( hardwareFilename, sizeof( hardwareFilename ), ".cfg" );

	if ( Cmd_Argc() == 3 ) {
		Q_strncpyz( replicateFilename, Cmd_Argv(2), sizeof( replicateFilename ) );
		COM_DefaultExtension( replicateFilename, sizeof( replicateFilename ), ".cfg" );
		Com_Printf( "Writing hardware cfg %s. Replicated cfg %s\n", hardwareFilename, replicateFilename );
		Com_WriteConfigToFile( hardwareFilename, replicateFilename, qfalse );
		return;
	}
...
Writes the retail Quake Live client-config subset to a specific filename.
===============
*/
void Com_WriteClientConfig_f( void ) {
	fileHandle_t	f;
	...
	if ( Cmd_Argc() != 2 ) {
		Com_Printf( "Usage: writeClientConfig <filename>\n" );
		return;
	}
...
void Field_CompleteCommand( field_t *field, fieldCompletionCallback_t callback ) {
	...
	command = Cmd_Argv( 0 );
	completionString = Cmd_Argv( argc - 1 );
	if ( completionString[0] == '\\' || completionString[0] == '/' ) {
		if ( completionString == command ) {
			prefix[0] = '\\';
		}
	...
	if ( argc < 2 && matchCount == 0 ) {
		return;
	}

	if ( !prefix[0] && command[0] != '\\' && command[0] != '/' ) {
		prefix[0] = '\\';
	}

	completionField->buffer[0] = '\0';
	if ( prefix[0] ) {
		Q_strcat( completionField->buffer, sizeof( completionField->buffer ), prefix );
	}
