...
zip files of the form "pak00.pk3", "pak01.pk3", etc.  Zip files are searched in decending order
from the highest number to the lowest, and will always take precedence over the filesystem.
This allows a pk3 distributed as a patch to override all existing data.

Quake Live does not use demo/oem productid restrictions. The filesystem can still be
forced into restricted mode by setting "+set fs_restrict 1" on the command line.
...
typedef struct pack_s {
	char			pakFilename[MAX_OSPATH];	// c:\quake3\baseq3\pak00.pk3
	char			pakBasename[MAX_OSPATH];	// pak00
...
=================
*/
static fileHandle_t FS_NormalizeHandle( fileHandle_t f ) {
#if defined(_WIN32)
	fileHandle_t candidate;
#endif
...
#if defined(_WIN32)
	if ( currentVM && currentVM->dllInterface ) {
		candidate = f;
		__try {
			candidate = *(fileHandle_t *)(intptr_t)f;
		} __except( EXCEPTION_EXECUTE_HANDLER ) {
			candidate = f;
		}
		if ( candidate >= 0 && candidate < MAX_FILE_HANDLES ) {
			return candidate;
		}
	}
#endif
...
=================
*/
static fileHandle_t FS_CheckedHandle( fileHandle_t f, const char *context ) {
	f = FS_NormalizeHandle( f );
	if ( f < 0 || f >= MAX_FILE_HANDLES ) {
		Com_Error( ERR_DROP, "%s: out of range", context );
	}
	return f;
}

static FILE	*FS_FileForHandle( fileHandle_t f ) {
	f = FS_CheckedHandle( f, "FS_FileForHandle" );
...
fs_webpath. These local launcher-style fallbacks stay available even when
live online services are disabled by build or runtime policy.
=============
*/
...
static qboolean FS_PreflightPathExists( const char *asset, const char *label, const char *rootPath )
{
	...
	for ( search = fs_searchpaths ; search ; search = search->next ) {
		if ( search->dir ) {
			if ( Q_stricmp( search->dir->path, rootPath ) ) {
				continue;
			}
			...
			if ( !FS_PathHasPrefix( search->pack->pakFilename, rootPath ) ) {
				continue;
			}

			if ( FS_PakFileExists( search->pack, asset ) ) {
				Com_sprintf( fullpath, sizeof( fullpath ), "%s:%s", search->pack->pakFilename, asset );
				exists = qtrue;
				break;
			}
		}
...
/*
	...
	Retries failed HTTP/resource requests using mapped filesystem paths, even in
	service-disabled builds where only the local fallback roots remain valid.
	=============
*/
...
void FS_FCloseFile( fileHandle_t f ) {
	...
	f = FS_CheckedHandle( f, "FS_FCloseFile" );
	if (fsh[f].streamed) {
...
Checks whether a normalized path begins with a normalized prefix.
===========
*/
static qboolean FS_PathHasPrefix( const char *path, const char *prefix ) {
	char	normalizedPath[MAX_OSPATH];
	char	normalizedPrefix[MAX_OSPATH];
	int		prefixLen;
	...
	Q_strncpyz( normalizedPath, path, sizeof( normalizedPath ) );
	Q_strncpyz( normalizedPrefix, prefix, sizeof( normalizedPrefix ) );

	FS_ConvertPath( normalizedPath );
	FS_ConvertPath( normalizedPrefix );

	prefixLen = (int)strlen( normalizedPrefix );
	while ( prefixLen > 0 && normalizedPrefix[prefixLen - 1] == '/' ) {
		normalizedPrefix[prefixLen - 1] = '\0';
		prefixLen--;
	}
	...
	if ( Q_stricmpn( normalizedPath, normalizedPrefix, prefixLen ) != 0 ) {
		return qfalse;
	}

	if ( normalizedPath[prefixLen] == '\0' || normalizedPath[prefixLen] == '/' ) {
		return qtrue;
	}
...
qboolean FS_idPak( char *pak, char *base ) {
	...
	for (i = 0; i < NUM_ID_PAKS; i++) {
		if ( !FS_FilenameCompare(pak, va("%s/pak%02d", base, i)) ) {
			break;
		}
	}
	if (i < NUM_ID_PAKS) {
...
static void FS_SetRestrictions( void ) {
	if ( fs_restrict && fs_restrict->integer ) {
		Com_Printf( "\nRunning with filesystem restrictions enabled.\n\n" );
		return;
	}
