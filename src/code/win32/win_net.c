...
static WSAEVENT	ip_socket_event = (WSAEVENT)INVALID_HANDLE_VALUE;
...
void NET_OpenIP( void ) {
	...
	if ( net_strict->integer ) {
		...
		hEventObject = CreateEventA( NULL, qfalse, qfalse, NULL );
		ip_socket_event = hEventObject;
		WSAEventSelect( ip_socket, hEventObject, FD_READ );
...
void NET_Config( qboolean enableNetworking ) {
	...
	if( stop ) {
		if ( ip_socket && ip_socket != INVALID_SOCKET ) {
			WSACloseEvent( ip_socket_event );
			ip_socket_event = (WSAEVENT)INVALID_HANDLE_VALUE;
...
void NET_Sleep( int msec ) {
	if ( ip_socket_event != (WSAEVENT)INVALID_HANDLE_VALUE ) {
		WSAWaitForMultipleEvents( 1, &ip_socket_event, qfalse, msec, qtrue );
	}
}
