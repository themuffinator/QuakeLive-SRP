...
HOST_ARCH="${QL_POSIX_ARCH:-$(uname -m | tr '-' '_')}"
CC="${QL_POSIX_CC:-${CC:-cc}}"
CXX="${QL_POSIX_CXX:-${CXX:-c++}}"
MAKE_JOBS="${QL_POSIX_MAKE_JOBS:-2}"
EXTRA_CFLAGS="${QL_POSIX_EXTRA_CFLAGS:-}"
CFLAGS_DEFAULT="-std=gnu89 -O2 -DNDEBUG -fPIC -fcommon -fsigned-char -include stdint.h -I${REPO_ROOT}/src/code -I${REPO_ROOT}/src ${HOST_CFLAGS_DEFAULT}"
...
COMMON_DEFINES="-DQL_BUILD_ONLINE_SERVICES=0 -DQL_BUILD_STEAMWORKS=0 -DQL_BUILD_OPEN_STEAM=0 -DQL_ENABLE_OGG=0"
